...
(function($) {
  ...
  ColorSelector.prototype = {
    ...
    _init : function() {
      ...
      var $markupSpanLine = $("<span>").addClass("line-colorselector").css("background-color", selectColor);
      var $markupA = $("<a>").attr("data-toggle", "dropdown").addClass("dropdown-toggle").attr("href", "#").append($markupSpan);
      var $markupALine = $("<a>").attr("data-toggle", "dropdown")
                                .attr("role", "button")
                                .addClass("dropdown-toggle")
                                //.addClass("btn")
                                .addClass("btn-default")
                                .attr("href", "#")
                                .append($markupSpanLine);
      ...
      this.$select.on("change", function() {
        ...
        $(this).next().find(".line-colorselector").css("background-color", color);

        callback($(this), value, color, title);
      });
  ...
  $.fn.colorselector.defaults = {
    callback : function(select, value, color, title) {
    },
    colorsPerRow : 8
  };
